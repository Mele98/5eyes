...
6. **CI-Golden-Snapshot-Nachtrag (2026-08-19), GELÖST:** Nach Punkt 5 zeigte sich ein zweiter, echter (kein Timeout) Testfehler: `test_golden_ch_snapshot_matches_frozen_fixture[bucket5_ausgewogen_default_prefs]` differierte in CI um wenige CHF/Basispunkte vom eingefrorenen Fixture (SLSQP-Optimierung). Erste Hypothese — unterschiedliche numpy/scipy-Versionen (lokal `2.4.4`/`1.17.1` vs. CI-frisch `2.5.2`/`1.18.0`, offene Ranges in `requirements.txt`) — wurde exakt gepinnt (`numpy==2.5.2`, `scipy==1.18.0`), zog aber einen Folgefehler nach sich: der separate `market_data_smoketest.yml`-Workflow lief noch auf Python 3.11, für das es keine numpy-2.5.2-Wheels gibt (Python ≥3.12 nötig) — auf 3.12 vereinheitlicht (jetzt konsistent mit dem Backend-Tests-Job). Nach dem Pinnen zeigte ein lokaler Repro unter identischen Versionen auf Windows aber weiterhin GRÜN gegen das alte Fixture — die Drift ist also keine reine Versionsfrage, sondern eine echte Windows-vs-Linux-Plattformdifferenz im SLSQP-Solver (vermutlich BLAS/LAPACK-Backend je Wheel), bestätigt durch triviale Verschiebungen (`expected_return_bps` 396→397, Beträge im Rappen- bis niedrigen-CHF-Bereich — reines Konvergenzrauschen, keine Logikänderung). Fix: temporärer CI-Schritt regenerierte die Fixtures direkt auf dem Linux-Runner (`_ci_regen_golden.py`, per Artefakt-Upload abgeholt, danach wieder entfernt); nur die eine betroffene Fixture-Datei wich tatsächlich ab, übernommen. Golden-Fixtures müssen künftig **immer** auf dem CI-Runner regeneriert werden, nie lokal unter Windows (Hinweis jetzt im Modul-Docstring von `test_golden_snapshot_ch_regression.py`).
