...
5. **CI-Performance-Nachtrag (2026-08-18):** Das PR-Gate "Backend Tests" braucht mit der gewachsenen Suite (5991 Tests) unter Coverage-Instrumentierung strukturell ~150min in GitHub Actions (~6x langsamer als lokal ohne Coverage, zwei unabhängige Läufe bestätigen das konsistent) — vermutlich weil `coverage.py`'s Line-Tracing die python-seitigen Monte-Carlo-/Optimizer-Schleifen besonders trifft. Sofort-Fix: `-v` entfernt, Timeout auf 120min gesetzt. Der eigentliche Fix steht noch aus: entweder `pytest-xdist` einführen (braucht vorher einen Parallel-Sicherheits-Audit der Suite — nicht blind aktiviert) oder das Coverage-Tracking aus dem blockierenden PR-Gate in einen separaten, nicht-blockierenden Job verschieben.
