...
- [✓] **8.** [DB] Postgres-Adapter verifizieren. **Bereits erledigt (ADR-012, Doc war veraltet):** `is_postgres_database_url`-Dialekt-Switch, `_create_or_migrate_schema` (alembic statt create_all unter Postgres, Roadmap #90). CI-Job mit echtem `postgres:16`-Service (`.github/workflows/test.yml:105-145`).
- [✓] **9.** [DB/SEC] Row-Level-Security-Policies. **Bereits erledigt (ADR-012, Doc war veraltet):** `services/postgres_rls.py` — `CREATE POLICY tenant_isolation ... USING (...) WITH CHECK (...)` pro mandantenführender Tabelle, `SET app.tenant_id` pro Connection (`services/tenant_context.py`, Pool-Checkout/Checkin-Reset), `_assert_rls_effective_role` blockt Boot in Staging/Prod bei superuser/BYPASSRLS-Rolle. Adversarial-Testsuite `tests/test_postgres_rls_adversarial.py` (Cross-Tenant-Insert/-Select, Pool-Reuse-Reset, Operator-Bypass) läuft in CI gegen echtes Postgres.
- [✓] **10.** [DB] `tenant_id` NOT NULL nach Backfill. **Bereits erledigt (ADR-012, Doc war veraltet):** `ensure_postgres_tenant_not_null()` (Backfill + `ALTER COLUMN ... SET NOT NULL`) bei jedem Postgres-Boot; SQLite bleibt bewusst nullable (Tier-1/3-Backwards-Compat).
- [✓] **11.** [DB/SEC] Per-Tenant-Encryption-Key (at-rest). **Bereits erledigt (Doc war veraltet):** `services/tenant_crypto.py`, `Tenant.encrypted_dek`/`dek_version`/`dek_rotated_at` (models/tenant.py). Tests: `tests/test_tenant_crypto.py`.
...
- [~] **24.** [OPS] Lizenz-/Quota-Enforcement. **Hard-Enforcement bereits erledigt (Doc war veraltet):** `services/quota.py::assert_within_quota` prüft max_users/max_mandates hart (409) an allen User-/Mandats-Anlegepunkten (`routers/mandates.py`, `routers/auth.py::create_user`+`invite_user`, `routers/clients.py::create_client_login` — letzteres war eine im Generalaudit 2026-07-25 gefundene Lücke, seither gefixt). 8 Tests (`test_tenant_quota_enforcement.py`). **Noch offen:** Soft-Limit-Warnschwelle (z.B. 80-90%) + Hinweis-UI im Frontend (aktuell 0 UI-Feedback vor dem harten 409); `storage_quota_mb` ist auf dem Tenant-Modell vorhanden aber unenforced (kein Storage-Feature existiert, das es verbrauchen würde — kein Bug, nur (noch) nichts zu begrenzen).
...
- [✓] **28.** [SEC] Session-/Token-TTL + Refresh. **Erledigt 2026-08-08 (Commit 541b16a):** Refresh-Token-Rotation mit Reuse-Detection. **Nachtrag 2026-08-09:** Backend-Rotation war fertig, aber vom Frontend nie aufgerufen — jede Session starb einfach bei access_token_expire_minutes (8h) ohne stille Erneuerung. FE-Wiring ergaenzt: API.fetch() versucht bei 401 zuerst stillen Refresh+Retry vor dem harten Logout; eigener Speicherplatz fuer den Refresh-Token (Electron: zweite verschluesselte Datei, Browser: eigener sessionStorage-Key). Tests: `test_frontend_refresh_token_wiring.py` (5), `main.smoke.test.js` erweitert.
