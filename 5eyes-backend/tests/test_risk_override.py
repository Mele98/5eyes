...
def _make_assessment(
    *,
    ...
    ra.knowledge_services_json = json.dumps({}) if with_schema_markers else None
    ra.knowledge_instruments_json = json.dumps({}) if with_schema_markers else None
    ra.income_sources_json = json.dumps([]) if with_schema_markers else None
    ra.answers = []
    return ra
...
def _make_full_answers():
    """Minimal complete answer list matching the current questionnaire schema."""
    payload = [
        (1, "Finanzdienstleistungen: {}", 0),
        (2, "Finanzinstrumente: {}", 0),
    ...
    answers = []
    for qn, label, points in payload:
        answer = MagicMock()
...
def _make_legacy_answers_with_all_numbers():
    payload = [
        (1, "Geldmarktinstrumente, ETF, Obligationen / Wandelanleihe, Anlagefonds, Aktien", 0),
        (2, "Ich wurde jeweils aktiv von einem Anlageberater unterstuetzt.", 0),
        (3, "Grosse Erfahrung - Ich habe schon mehrfach Finanzdienstleistungen verwendet.", 0),
        (4, "Jaehrlich CHF 380'000 - Herkunft: Aus beruflicher Taetigkeit", 2),
        (5, "12 Jahre und mehr - Matrix-Faktor", 0),
        (6, "Jaehrlich CHF 290'000", 0),
        (7, "Mehr als 10 % bis 25 %", 8),
        (8, "Bis zu 12 Monatsauslagen", 6),
        (9, "Das investierte Kapital soll sich stetig vermehren.", 3),
        (10, "Ich strebe eine hoehere Rendite an und bin bereit, dafuer ein erhoehtes Risiko einzugehen.", 3),
        (11, "Ich kann den Verlust voruebergehend akzeptieren und halte an meinen Anlagen fest.", 3),
    ]
...
def test_documented_override_with_score_is_strategy_ready_even_without_answers():
    """Documented override with explicit score bypasses answer completeness."""
    ra = _make_assessment(
        is_overridden=1,
...
def test_small_or_downward_override_with_reason_is_strategy_ready():
    ra = _make_assessment(
        ...
        final_score_x10=70,
        override_reason="Konservativer Override dokumentiert",
    )
    assert risk_assessment_ready_for_strategy(ra) is True
