...
async function loadClientById(clientId) {
  ...
  try {
    ...
    if (clientRes.status === 'fulfilled') {
      ...
      strategyState={allocation:null,recommendation:null,activeMandateId:currentMandateId,lastGeneratedAt:0,loading:false,dirty:true,risk:null};
...
let strategyState={allocation:null,recommendation:null,activeMandateId:null,lastGeneratedAt:0,loading:false,dirty:true,risk:null};

function buildBudget(){/* Stub: Budget-Seite entfernt. buildAT() ruft diese Funktion — kein Seiteneffekt. */}

function buildQT(id){
  const t=document.getElementById(id);if(!t)return;
  t.innerHTML=`<div style="display:grid;grid-template-columns:8px 1fr 42px 62px 62px 62px;gap:0;padding-bottom:4px;border-bottom:2px solid var(--b2);margin-bottom:2px;"><div></div><div style="font-size:9px;font-weight:500;letter-spacing:0.05em;text-transform:uppercase;color:var(--n4);padding-left:6px">Asset-Klasse</div><div style="font-size:9px;font-weight:500;text-transform:uppercase;letter-spacing:0.05em;color:var(--n4);text-align:right">Ist</div><div style="font-size:9px;font-weight:500;text-transform:uppercase;letter-spacing:0.05em;color:var(--n4);text-align:right">Min</div><div style="font-size:9px;font-weight:500;text-transform:uppercase;letter-spacing:0.05em;color:var(--n4);text-align:right">Ziel</div><div style="font-size:9px;font-weight:500;text-transform:uppercase;letter-spacing:0.05em;color:var(--n4);text-align:right">Max</div></div>`+quoten.map(r=>`<div class="qtr"><div class="qtd" style="background:${r.c}"></div><div class="qtn">${r.n}</div><div class="qti">${r.ist}%</div><input class="qtinp" value="${r.min}%"><input class="qtinp" value="${r.ziel}%" style="border-color:var(--n6)"><input class="qtinp" value="${r.max}%"></div>`).join('');
}
...
function initCharts(){
  ...
  const yr=[2025,2026,2027,2028,2029,2030,2031,2032,2033,2034,2035];
  const ist=[2848,2990,3140,3295,3460,3630,3810,4000,4195,4400,4615];
  const opt=[2848,3050,3270,3510,3760,4030,4320,4630,4960,5310,5690];
  ...
  buildAT();buildQT('qt-modal');buildBudget();setupDrag();
}
...
function printReport(){
  ...
  if(p)document.title='5eyes Report \u2014 '+p.name+' \u2014 '+new Date().toLocaleDateString('de-CH');
  window.print();
  setTimeout(()=>{document.title='5eyes \u00b7 WealthArchitekten';},3000);
}

function openPortfolioPositionHelper(){
  var title=document.querySelector('#m-ap .mtitle');
  var body=document.querySelector('#m-ap .mbody');
  var footer=document.querySelector('#m-ap .mfooter');
  if(title)title.textContent='Portfolio-Positionen entstehen aus dem Rezept';
  if(body){
    body.innerHTML='<div style="background:var(--bg);border:1px solid var(--b1);border-radius:var(--r);padding:10px 12px;font-size:11px;color:var(--n6);line-height:1.6">'
      +'Produktpositionen im Portfolio werden aus der Soll-Allokation und dem Anlagerezept aufgebaut. '
      +'Wenn du einen Vermögenswert erfassen willst, nutze die Erfassung im Gesamtvermögen. '
      +'Wenn du neue Titel für das Advisory-Portfolio willst, baue zuerst das aktuelle Rezept live auf.'
      +'</div>';
  }
  if(footer){
    footer.innerHTML='<button class="btn" onclick="cm(\'m-ap\')">Schliessen</button>'
      +'<button class="btn" onclick="cm(\'m-ap\');go(\'ub\');om(\'m-aw\')">Zum Gesamtvermögen</button>'
      +'<button class="btn-p" onclick="cm(\'m-ap\');openImplementationRecipe()">Rezept aufbauen</button>';
  }
  om('m-ap');
}
...
function bindHonestUiActions(){
  ...
  if(reportBtn){
    ...
    reportBtn.setAttribute('onclick',"cm('m-rep');printReport()");
  }
  ...
  if(riskPdfBtn){
    ...
    riskPdfBtn.setAttribute('onclick',"cm('m-rp-print');printReport()");
  }
...
function loadAllocationPreferences(){
  ...
  try{
    ...
    if(raw){
      ...
      if(stored&&typeof stored==='object')prefs=stored;
    }
  }catch(e){}
...
function persistAllocationPreferences(){
...
function renderActionCards(buckets,reasoning,totalRappen){
  var el=document.getElementById('aa-action-list');
  if(!el)return;
  var items=(Array.isArray(buckets)?buckets.slice():[]).sort(function(a,b){
    return Math.abs(Number(b.delta_weight_bps||0))-Math.abs(Number(a.delta_weight_bps||0));
  }).filter(function(item){return Math.abs(Number(item.delta_weight_bps||0))>=100;}).slice(0,4).map(function(item){
    var up=Number(item.delta_weight_bps||0)>0;
    var severity=Math.abs(Number(item.delta_weight_bps||0))>=300?'Hoch':(Math.abs(Number(item.delta_weight_bps||0))>=150?'Mittel':'Tief');
    var tagClass=severity==='Hoch'?'tr2':(severity==='Mittel'?'ta':'tn');
    var amount=Math.round((Number(totalRappen||0)*Math.abs(Number(item.delta_weight_bps||0))/10000)/100);
    var verb=up?'aufbauen':'reduzieren';
    return '<div style="border:1px solid var(--b1);border-radius:var(--r);padding:8px 10px"><div style="display:flex;justify-content:space-between;margin-bottom:3px"><span style="font-size:11px;font-weight:500">'+assetDisplayName(item.asset_class)+' '+verb+'</span><span class="tag '+tagClass+'">'+severity+'</span></div><div style="font-size:10px;color:var(--n6);line-height:1.5">'+formatBpsPercent(Math.abs(item.delta_weight_bps))+' Drift. Ca. CHF '+amount.toLocaleString('de-CH')+' Richtung Soll.</div></div>';
  });
...
function renderPortfolioSections(positions){
  ...
  wrap.innerHTML=Object.keys(groups).map(function(group){
    ...
    var subtotal=rows.reduce(function(sum,row){return sum+Number(row.target_amount_rappen||0);},0);
    var pct=rows.reduce(function(sum,row){return sum+Number(row.target_weight_bps||0);},0);
    var rowHtml=rows.map(function(row){
      return '<div class="prow"><div class="pisin">'+escapeHtml(row.isin||'–')+'</div><div><div class="pname">'+escapeHtml(row.product_name||'Produkt')+'</div><div class="pdet">'+escapeHtml((row.provider||'')+(row.currency?' · '+row.currency:'')+(row.rationale?' · '+row.rationale:''))+'</div></div><div><span class="tag tn">'+escapeHtml(row.sub_asset_class||row.asset_class||'Titel')+'</span></div><div class="pval">'+formatBpsPercent(row.target_weight_bps)+'</div><div class="pval">'+Math.round((Number(row.target_amount_rappen||0))/100).toLocaleString('de-CH')+'</div><div class="pval" style="color:var(--n4)">–</div><div class="pperf" style="color:var(--n4)">–</div><div class="pval" style="font-size:9px;color:var(--n4)">'+((row.ter_bps||0)/100).toFixed(2)+'%</div></div>';
    }).join('');
...
function renderRecommendationWarnings(warnings){
...
function renderStrategySummary(){
  ...
  var projectedRappen=projectionValueRappen(
    allocation.advisory_wealth_rappen,
    allocation.cashflow_projection_series_rappen||allocation.annual_net_cashflow_rappen,
    allocation.expected_return_bps,
    7
  );
  var avgGoalScore=allocation.goal_analysis&&allocation.goal_analysis.length
    ? Math.round(allocation.goal_analysis.reduce(function(sum,item){return sum+Number(item.achievement_score||0);},0)/allocation.goal_analysis.length)
    : 0;
  ...
  setText('sr-goal-sub',endangered?endangered+' Ziel(e) gefährdet':'Ziele mehrheitlich on track');
  setText('sr-proj-wealth',formatCompactCHF(projectedRappen/100));
  setText('sr-proj-sub','Nach Umsetzung Soll');
...
function applyAllocationEngineResult(result){
  ...
  quoten=(result.buckets||[]).map(function(bucket){
    var key=String(bucket.asset_class||'');
    return {n:assetDisplayName(key),c:colors[key]||'#5a6878',ist:Number(bucket.current_weight_bps||0)/100,min:Number(bucket.band_min_bps||0)/100,ziel:Number(bucket.target_weight_bps||0)/100,max:Number(bucket.band_max_bps||0)/100};
  });
  ...
  if(charts.opt&&charts.opt.data&&charts.opt.data.labels){
    var years=Math.max(1,charts.opt.data.labels.length-1);
    var projectionInput=result.cashflow_projection_series_rappen||result.annual_net_cashflow_rappen;
    charts.opt.data.datasets[0].data=buildProjectionSeriesK(result.advisory_wealth_rappen,projectionInput,result.expected_return_bps,years);
    if(charts.opt.data.datasets[1])charts.opt.data.datasets[1].data=buildProjectionSeriesK(
      result.advisory_wealth_rappen,
      scaleProjectionContribution(projectionInput,0.7),
      Math.max(0,Number(result.expected_return_bps||0)-120),
      years
    );
    charts.opt.update();
  }
  var projectedRappen=projectionValueRappen(
    result.advisory_wealth_rappen,
    result.cashflow_projection_series_rappen||result.annual_net_cashflow_rappen,
    result.expected_return_bps,
    7
  );
  var deltaChf=Math.round((projectedRappen-Number(result.advisory_wealth_rappen||0))/100);
  ...
  setText('aa-proj-cagr',((Number(result.expected_return_bps||0)/100).toFixed(2))+'%');
  setText('aa-proj-vol',((Number(result.expected_volatility_bps||0)/100).toFixed(2))+'%');
  renderActionCards(result.buckets,result.reasoning,result.advisory_wealth_rappen);
  renderStrategySummary();
}
function applyRecommendationEngineResult(result){
  strategyState.recommendation=result;
  setText('po-advisory-wealth',formatRappen(result.advisory_wealth_rappen||0));
  setText('po-total-footer',formatRappen(result.advisory_wealth_rappen||0));
  ...
  renderRecommendationWarnings(result.warnings||[]);
  renderPortfolioSections(result.positions||[]);
  renderStrategySummary();
}
async function refreshStrategyData(force,includeRecommendation){
  var mid=getActiveMandateId();
  ...
  try{
    ...
    var results=await Promise.all([allocationPromise,riskPromise]);
...
renderStrategySummary=function(){
  ...
  goals.innerHTML=(allocation.goal_analysis||[]).slice(0,3).map(function(goal){
    ...
    return '<div style="display:flex;justify-content:space-between;align-items:flex-start;gap:12px;padding:8px;background:var(--bg);border-radius:var(--r);border:1px solid var(--b1)">'+
      '<div><div><span style="font-size:9px;font-weight:600;color:var(--n5);margin-right:6px">RANG '+escapeHtml(goal.rank)+'</span><span style="font-size:11px;color:var(--n8)">'+escapeHtml(goal.label||'Ziel')+'</span></div><div style="font-size:10px;color:var(--n6);margin-top:4px">'+escapeHtml(goalTimingLabel(goal))+'</div></div>'+
      '<div style="text-align:right"><div style="font-size:10px;font-weight:600;color:var(--n7)">'+escapeHtml(strategyGoalTargetText(goal))+'</div><div style="font-size:10px;color:'+tone+';margin-top:4px">'+escapeHtml(String(score))+'%</div></div></div>';
  }).join('');
};
