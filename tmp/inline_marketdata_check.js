...
function renderEngineRuntimePanels(result){
  ...
  function summarizeProviderRole(role){
    if(!role||typeof role!=='object')return '';
    var current=role.current||{};
    var target=role.target||{};
    function entryLabel(entry){
      if(!entry||typeof entry!=='object')return '';
      var label=String(entry.label||entry.name||'').trim();
      if(!label)return '';
      if(entry.api_key_required&&!entry.api_key_present)return label+' (API-Key offen)';
      if(entry.contract_required&&!entry.configured)return label+' (Vertrag/API-Key offen)';
      return label;
    }
    var currentBits=[];
    var targetBits=[];
    var currentPrimary=entryLabel(current.primary);
    var currentFallback=entryLabel(current.fallback);
    var targetPrimary=entryLabel(target.primary);
    var targetFallback=entryLabel(target.fallback);
    if(currentPrimary)currentBits.push(currentPrimary);
    if(currentFallback)currentBits.push('Fallback '+currentFallback);
    if(targetPrimary)targetBits.push(targetPrimary);
    if(targetFallback)targetBits.push('Fallback '+targetFallback);
    if(!currentBits.length&&!targetBits.length)return '';
    return String(role.label||'Provider')
      +': '
      +(currentBits.length?('heute '+currentBits.join(' / ')):'')
      +(currentBits.length&&targetBits.length?' / ':'')
      +(targetBits.length?('ziel '+targetBits.join(' / ')+(target.wired===false?' (noch nicht verdrahtet)':'')):'');
  }
  ...
  var providerRoles=runtime&&runtime.provider_roles?runtime.provider_roles:null;
  var providerRoleLines=providerRoles?[
    summarizeProviderRole(providerRoles.market_prices),
    summarizeProviderRole(providerRoles.reference_data),
    summarizeProviderRole(providerRoles.id_mapping),
    summarizeProviderRole(providerRoles.macro_core),
    summarizeProviderRole(providerRoles.macro_switzerland),
    summarizeProviderRole(providerRoles.enterprise_reference)
  ].filter(Boolean):[];
  var marketItems=[
  ...
  if(providerRoleLines.length){
    marketItems=marketItems.slice(0,2).concat(providerRoleLines,marketItems.slice(2));
  }
  if(quality){
    marketItems.splice(2+providerRoleLines.length,0,
      ...
      'Lookup-Qualitaet: Symbol '+String(quality.direct_symbol_lookup_products_count||0)+' / nur ISIN '+String(quality.direct_isin_lookup_products_count||0)+' / proxy '+String(quality.proxy_lookup_products_count||0)+' / synthetisch '+String(quality.synthetic_lookup_products_count||0)+' / Luecken '+String(quality.mapping_gap_count||0)+(quality.latest_price_date?(' / Letztes Preisdatum '+String(quality.latest_price_date)):'')
    );
  }
